...
def _persist_report(session: Session, source: SourceDocument, parsed: Any) -> None:
    ...
    source.metadata_json = {**parsed.metadata, "pdf_version": parsed.pdf_version,
                            "sentinel_count": parsed.sentinel_count}
    for item in parsed.pages:
        session.add(Page(
            source_document_id=source.id, page_number=item.page_number, width=item.width,
            height=item.height, native_character_count=item.native_character_count,
            deduplicated_character_count=item.deduplicated_character_count,
            raw_text=item.text, extraction_method="native_pdf", confidence=1.0,
        ))
    report = Report(
        source_document_id=source.id, wellbore=parsed.wellbore,
        filename_wellbore=parsed.filename_wellbore, period_start=parsed.period_start,
        period_end=parsed.period_end, filename_date=parsed.filename_date, spud_date=parsed.spud_date,
        report_number=parsed.report_number, status_raw=parsed.status_raw,
        summary_activities=parsed.summary_activities, summary_planned=parsed.summary_planned,
        filename_identity_match=parsed.filename_identity_match, filename_date_match=parsed.filename_date_match,
    ...
    for item in parsed.sections:
        session.add(ReportSection(
            report_id=report.id, section_type=item.section_type, heading_raw=item.heading_raw,
            page_number=item.page_number, text=item.text, row_count=item.row_count,
    ...
    for item in parsed.operations:
        session.add(Operation(
            report_id=report.id, row_index=item.row_index, page_number=item.page_number,
            start_time_raw=item.start_time_raw, end_time_raw=item.end_time_raw,
            duration_hours=item.duration_hours, end_depth_mmd_raw=item.end_depth_raw,
            end_depth_mmd=item.end_depth_mmd, end_depth_missing_reason=item.end_depth_missing_reason,
    ...
    for item in parsed.fields:
        ...
        session.add(ExtractedValue(
            source_document_id=source.id, page_number=item.provenance.page_number,
            section_type=item.provenance.section, field_name=item.field_name, raw_value=item.raw_value,
            normalized_text=item.normalized_text, normalized_number=item.normalized_number,
            unit_raw=item.unit_raw, unit_normalized=item.unit_normalized, missing_reason=item.missing_reason,
            bbox_json=None if bbox is None else dict(
                zip(("x0", "top", "x1", "bottom"), bbox, strict=True)
            ),
            confidence=item.confidence,
        ))
    for warning in parsed.warnings:
        session.add(Anomaly(
            source_document_id=source.id, source_record_type="report", source_record_id=report.id,
            category="source_data_quality", rule_or_model=str(warning["code"]), evidence_json=warning,
            score=1.0, severity_heuristic=str(warning.get("severity", "medium")), confidence=1.0,
            threshold_json={}, domain_validated=False,
            explanation="Automated data-quality candidate; requires human/domain review.",
        ))


def _unresolved_mapping(session: Session, source_namespace: str, source_identifier: str,
                        target_namespace: str, note: str) -> None:
    existing = session.scalar(select(IdentityMapping).where(
    ...
    if not existing:
        session.add(IdentityMapping(
            source_namespace=source_namespace, source_identifier=source_identifier,
            target_namespace=target_namespace, target_identifier="unresolved",
            mapping_status="unresolved", mapping_source="no_manifest_or_metadata",
            evidence="No authoritative mapping metadata was supplied.", confidence=0.0,
            validation_status="unreviewed", notes=note,
        ))
...
def _persist_plot(session: Session, source: SourceDocument, result: dict[str, Any]) -> None:
    ...
    for item in result["points"]:
        ...
        session.add(PlotPoint(
            plot_id=plot.id, point_index=item["point_index"],
            series_identifier=item["series_identifier"], pixel_x=item["pixel_x"], pixel_y=item["pixel_y"],
            x_value=item.get("pressure") if result["plot_type"] == "pressure_profile" else None,
            y_value=item.get("depth") if result["plot_type"] == "pressure_profile" else item.get("pressure"),
            ...
            anomaly_candidate=item.get("anomaly_candidate", False), confidence=item["confidence"],
            source_bbox_json=item["source_bbox"],
        ))
        if item.get("anomaly_candidate"):
            session.add(Anomaly(
                source_document_id=source.id, source_record_type="plot_point", source_record_id=None,
                category="profile_reference_band_candidate", rule_or_model="pixel_curve_relative_position",
                evidence_json={"plot": result["plot_identifier"], "point": item}, score=1.0,
                severity_heuristic="medium", confidence=item["confidence"], threshold_json={},
    ...
    if result["plot_type"] == "pressure_profile":
        _unresolved_mapping(session, "pressure_profile", result["plot_identifier"], "ddr_wellbore",
                            "Generic profile identifiers must not be mapped to DDR wellbores by index.")
        _unresolved_mapping(session, "pressure_profile", result["plot_identifier"], "pressure_time_plot",
                            "Profile and pressure-time filename indices are not established as identities.")
    else:
        _unresolved_mapping(session, "pressure_time_plot", result["plot_identifier"], "pressure_profile",
                            "Pressure-time filename index must not be mapped to a profile index.")
        _unresolved_mapping(session, "pressure_time_plot", result["plot_identifier"], "ddr_wellbore",
...
def process_file(path: str | Path) -> dict[str, Any]:
    ...
    with session_scope() as session:
        source, unchanged = _source_for_path(session, source_path, digest, decision.kind.value)
        if unchanged:
            return {"path": str(source_path), "status": "skipped_unchanged", "sha256": digest}
        job = ProcessingJob(
            source_document_id=source.id, job_type=f"process_{decision.kind.value}", status="running",
            parser_version=settings.parser_version,
        )
        session.add(job)
        session.flush()
        try:
            if decision.kind == AssetKind.DIGITAL_PDF:
                ...
                overlay = settings.processed_dir / "overlays" / f"{source_path.stem}_overlay.png"
        ...
        return {"path": str(source_path), "status": job.status, "sha256": digest,
                "duration_seconds": job.duration_seconds, "error": job.error_message}


def process_paths(paths: list[Path]) -> list[dict[str, Any]]:
    return [process_file(path) for path in paths]
