...
def _process_document(
    document_row: SourceDocument, report: Report, database_url: str
) -> dict[str, object]:
    ...
    with session_scope(database_url) as session:
        ...
        if existing:
            session.execute(
                delete(SectionTableRow).where(SectionTableRow.source_document_id == document_row.id)
            )
        ...
        for item in output_rows:
            session.add(
                SectionTableRow(
                    ...
                    report_id=report.id,
                    report_section_id=section_lookup.get(
                        (item["page_number"], item["section_type"])
                    ),
                    page_number=item["page_number"],
                    section_type=item["section_type"],
                    table_index=item["table_index"],
                    row_index=item["row_index"],
                    header_cells_json=item["headers"],
                    raw_cells_json=item["cells"],
                    normalized_cells_json=item["normalized"],
                    table_bbox_json=item["bbox"],
                    confidence=0.92,
                )
            )
...
def main() -> None:
    ...
    for index, (document, report) in enumerate(documents, start=1):
        try:
            result = _process_document(document, report, settings.database_url)
