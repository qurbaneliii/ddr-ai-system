...
from ddr_ai.db.session import get_engine
...
def test_failure_chat_renders_complete_table_and_csv_download(monkeypatch, tmp_path: Path) -> None:
    ...
    app.radio[0].set_value("Chatbot").run(timeout=60)
    ...
    assert not app.exception
    assert len(app.chat_message) == 2
    assert "242 populated equipment-failure records" in app.chat_message[-1].markdown[0].value
    assert app.dataframe[-1].value.shape == (242, 17)
